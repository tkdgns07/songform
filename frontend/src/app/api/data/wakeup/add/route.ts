...
export async function POST(request: NextRequest) {
    const response = await axios.post(`${process.env.NEXTAUTH_URL}/api/data/wakeup/check`, {
        request
    })
    if (response.data){ return NextResponse.json({ status: 404, error: 'already-submit' }); }
    try {
